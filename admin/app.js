...
function memberCompleteness(m){const checks=[['基本資料',!!(m.name&&m.company&&m.tagline)],['專業資料',!!(m.expertise_category&&m.services&&m.common_problems&&m.work_scope)],['自我介紹',!!String(m.intro||'').trim()],['電子名片',!!String(m.card_json_url||'').trim()],['聯絡方式',(memberContactCounts.get(m.id)||0)>0],['媒合',(memberMatchCounts.get(m.id)||0)>0]];return{done:checks.filter(x=>x[1]).length,total:checks.length,missing:checks.filter(x=>!x[1]).map(x=>x[0])}}
function renderMemberList(){const el=$('memberList');const scores=members.map(m=>({m,c:memberCompleteness(m)}));const completeCount=scores.filter(x=>x.c.done===x.c.total).length;const summary=$('memberCompletenessSummary');if(summary)summary.innerHTML=`<div><b>資料完整度統整</b><span>共 ${members.length} 位成員</span></div><div class="completeness-summary-counts"><span class="complete">完整 ${completeCount} 位</span><span class="incomplete">待補 ${members.length-completeCount} 位</span></div>`;el.innerHTML=scores.map(({m,c})=>{const complete=c.done===c.total;return `<button class="list-item ${currentMember?.id===m.id?'active':''}" data-member="${m.id}"><div class="member-list-top"><b>${m.member_no||'-'}｜${esc(m.name)}</b><span class="completeness-badge ${complete?'complete':'incomplete'}">資料完整度 ${c.done}/${c.total}</span></div><span class="member-list-meta">${esc(m.company||'')}</span></button>`}).join('');el.querySelectorAll('[data-member]').forEach(b=>b.onclick=()=>selectMember(b.dataset.member))}
function clearMemberForm(){['m_name','m_company','m_title','m_tagline','m_expertise','m_services','m_common_problems','m_work_scope','m_intro','m_card_json_url'].forEach(id=>$(id).value='');$('m_visible').checked=true;$('memberNoText').textContent='建立後自動編號'}
