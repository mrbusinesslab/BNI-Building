...
function renderMemberList(){const el=$('memberList');el.innerHTML=members.map(m=>{const c=memberCompleteness(m),complete=c.done===c.total;return `<button class="list-item ${currentMember?.id===m.id?'active':''}" data-member="${m.id}"><div class="member-list-top"><b>${m.member_no||'-'}｜${esc(m.name)}</b><span class="completeness-badge ${complete?'complete':'incomplete'}">資料完整度 ${c.done}/${c.total}</span></div><span class="member-list-meta">${esc(m.company||'')}｜${m.is_published===false?'前台隱藏':'前台顯示'}</span></button>`}).join('');el.querySelectorAll('[data-member]').forEach(b=>b.onclick=()=>selectMember(b.dataset.member))}
